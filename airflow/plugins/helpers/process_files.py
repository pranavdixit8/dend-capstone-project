...
class SparkProcess:
	...
	def find_visa_category(self, category):
 ...
 #        return spark
	...
	def get_port_to_city_dict(self,file):
	...
	def get_city_to_port_dict(self,file):
		port_to_city_dict = self.get_port_to_city_dict(file)
	...
	def duration_in_days(self, startdate, enddate):
		try:
	...
	def process_immigration_data(self, spark, input_data, output_data, port_to_city_dict):
		"""
		...
		udf_find_visa_category = udf(lambda x: self.find_visa_category(x))
		df = df.withColumn("i94visa", udf_find_visa_category(df.i94visa))

		#convert arrival and depature date to date
		udf_convert_to_date = udf(lambda x : self.convert_to_date(x), DateType())
		...
		udf_duration_in_days  = udf(lambda x,y : self.duration_in_days(x,y))
	...
	def process_port_data(self, spark, input_data, output_data, city_to_port_dict):
		"""
	...
	def process_data(self):
		...
		input_labels_data = '/home/pranav/Desktop/data/projects/dend-capstone-project/data/I94_SAS_Labels_Descriptions.SAS'
		output_data = "s3a://dend-2020-capstone/"



		port_to_city_dict = self.get_port_to_city_dict(input_labels_data)
		city_to_port_dict = self.get_city_to_port_dict(input_labels_data)


		self.process_immigration_data(spark, input_immigration_data, output_data, port_to_city_dict)    
		self.process_port_data(spark, input_port_data, output_data, city_to_port_dict)
